# Windows 人工验收清单

> 自动化测试不能证明以下真实桌面行为。发布前在 Windows 11 x64 的 Release self-contained 构建逐项勾选并记录机器/DPI。

## 分层窗口与交互

- [ ] 宠物背景真实透明，没有黑/白矩形或边缘黑边。
- [ ] 透明区域点击落到后方程序，身体区域可单击、双击和拖拽。
- [ ] 拖出窗口后仍保持捕获，松开可靠结束拖拽。
- [ ] 完全穿透时窗口不接收输入，并能从托盘恢复。
- [ ] 宠物不出现在任务栏和 Alt+Tab，显示时不抢当前输入焦点。
- [ ] 总在最前开关即时生效。
- [ ] 隐藏不退出，托盘显示能恢复；托盘退出后进程消失。

## 显示器与系统

- [ ] 主/副显示器间移动正常，包括左侧负坐标显示器。
- [ ] 100%/125%/150% 不同 DPI 显示器之间尺寸和命中正确。
- [ ] 拔除保存位置所在显示器后，宠物回到主屏工作区且部分可见。
- [ ] 睡眠/唤醒后动画、位置和托盘正常。
- [ ] Explorer 重启后托盘图标恢复。
- [ ] Windows 注销/关机保存状态并无异常阻塞。

## 资产与生命周期

- [ ] 全部九种动画逐项播放，帧序和完成跳转正确。
- [ ] 导入合法目录和 ZIP 成功；非法包一次展示全部问题。
- [ ] 同 ID 的替换/取消正确，模拟失败时旧宠物仍可用。
- [ ] 当前宠物文件损坏后重启可回退；运行中切换失败仍显示旧宠物。
- [ ] 单实例：第二次启动只激活已有实例，且 import 命令有效。
- [ ] 设置关闭不退出；缩放、渲染模式、阈值、Idle、启动项实时生效。

## 安装与资源

- [ ] 从 GitHub Release 下载安装包、免安装 ZIP 和 `SHA256SUMS.txt`，两个包的 SHA-256 均匹配。
- [ ] 免安装 ZIP 解压后可直接启动，且苏筱、田偌和 Sweeper-EX 三个可选内置宠物均存在，小豆人故障回退资源也存在。
- [ ] 当前用户安装、开始菜单、可选桌面快捷方式正确。
- [ ] 覆盖安装成功；卸载默认保留配置/宠物，选择清理时删除用户数据。
- [ ] Release 静置、隐藏、暂停时 CPU 接近空闲，无持续重绘。
- [ ] 连续运行/切换 30 分钟，GDI Objects 和内存无持续增长。

## 在线宠物

- [ ] 未配置目录时在线页保持可用且不发起请求；填写有效 HTTPS SDCWeb 目录后能刷新名称、描述、作者、版本、大小、状态和预览。
- [ ] 无网络、超时、HTTP 错误、无效 JSON 和预览失败均显示失败状态，当前桌宠继续运行，本地切换和托盘退出不受影响。
- [ ] 安装合法在线宠物后立即可切换，托盘与设置列表同步更新；重新安装同 ID 后仍可加载。
- [ ] 客户端版本不足的条目标记为不兼容且不能安装；已安装但后来不兼容的条目仍可删除。
- [ ] 删除非当前宠物不切换窗口；删除当前宠物先切换默认内置宠物；内置宠物不能删除或被在线包覆盖。
- [ ] 关闭设置窗口时正在进行的目录/预览刷新会取消，不残留 `Staging\online-*` 事务目录。

## 2026-07-18 自动辅助证据

> 以下结果来自 Win32 诊断，不替代上方仍未勾选的肉眼与长时间人工验收。

- Release 桌宠窗口为 `192x208`，位于 `DISPLAY1` 的 100% DPI（96 DPI）1920x1080 显示器。
- 窗口同时具有 `WS_EX_LAYERED`、`WS_EX_TOOLWINDOW`、`WS_EX_NOACTIVATE` 和 `WS_EX_TOPMOST`，且不具有 `WS_EX_APPWINDOW`。
- `WM_MOUSEACTIVATE` 返回 `MA_NOACTIVATE`，测试前后前台窗口句柄保持不变。
- 13x13 像素命中采样得到 38 个实体点和 131 个透明点，完全穿透处于关闭状态。
- 30 分钟持续运行样本内 GDI Objects `28 -> 28`（峰值 28），USER Objects `33 -> 33`（峰值 33），工作集 `82.3 -> 83.2 MiB`（峰值 83.2 MiB），私有内存 `34.2 -> 35.2 MiB`（峰值 35.2 MiB），归一化平均 CPU 为 0.035%。该样本未包含宠物切换、隐藏或暂停，因此对应组合人工项仍保持未勾选。
- 受控输入曾使窗口从保存位置产生实际水平位移并触发位置保存，测试结束后窗口与设置均恢复到 `(1609, 641)`；输入工具会合并最后一个移动事件，因此未将完整拖拽项勾选为通过。
- `dotnet build SeedDormitoryCorridor.sln -c Release --no-restore` 成功，0 warnings、0 errors。
- `dotnet test SeedDormitoryCorridor.sln -c Release --no-build` 成功，41 passed。
- `dotnet format SeedDormitoryCorridor.sln --verify-no-changes --no-restore` 成功。

## 2026-07-18 人工验收补充

- 用户肉眼确认透明边缘正常，没有黑边；透明区域穿透正常；常态下任务栏和 Alt+Tab 均不出现宠物窗口。
- Sweeper-EX 手动播放多个动画无响应，记录为 #6；根因是手动菜单请求仍受运行时优先级阻塞。
- 宠物右键菜单显示期间任务栏出现应用图标，记录为 #7；根因是菜单通过无 owner 的顶层窗口显示。
- 多屏往返、负坐标、拔屏、睡眠/唤醒、Explorer 重启、注销/关机和安装生命周期验收延期到 #8，本轮不判定通过。
